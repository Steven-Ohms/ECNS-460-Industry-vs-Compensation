...
industry_data_annual<-industry_data_annual|>
  filter("NAICS" != 221) #remove a duplacite of utilities.
```
...
```{r}
t<-subset(industry_data_separated, industry_data_separated$NAICS==22)
...
cleaned_industry_labor_data|>
 ...
 t <-cleaned_industry_labor_data|>
   select("laborProductivity", "industry", "Average Salary", "Hours worked_Millions of hours", "Sectoral output_Millions of current dollars")
...
```{r}
cleaned_industry_labor_data$`Labor Productivity`<- cleaned_industry_labor_data$`Sectoral output_Millions of current dollars`/ (cleaned_industry_labor_data$`Employment_Thousands of jobs`*1000)


so<-cleaned_industry_labor_data$`Sectoral output_Millions of current dollars`*1000000
emp<-cleaned_industry_labor_data$`Employment_Thousands of jobs`*1000

ggplot(cleaned_industry_labor_data, aes(x = `Labor Productivity`, y = `Average Weekly Wages`))+geom_point()
